...
def getInput():
    ...
    SYSTOLIC = validate_input("Enter your top number (SYSTOLIC blood pressure in mm Hg: ")
    DIASTOLIC = validate_input("Enter your bottom number (DIASTOLIC) blood pressure in mm Hg: ")
...
def checkInputIfRejectable(SYSTOLIC, DIASTOLIC):
    ...
    if SYSTOLIC > MAXIMUM_SYSTOLIC or SYSTOLIC < MINIMUM_SYSTOLIC or DIASTOLIC > MAXIMUM_DIASTOLIC or DIASTOLIC < MINIMUM_DIASTOLIC:
        return("You have entered an invalid value for the blood pressure. Please enter values within the acceptable range.")
    
    return False

def processInput(SYSTOLIC,DIASTOLIC):
...
def main(): # MAIN function to be invoked for the program to work
    ...
    if rejectData: # Input was outside acceptabed range but it was numerical
        ...
        return(processInput(SYSTOLIC, DIASTOLIC))
